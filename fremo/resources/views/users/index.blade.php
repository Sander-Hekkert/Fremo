...
@if(array_key_exists(2, $usersByRoleArray))
    <h2>Gebruikers</h2>
    @foreach($usersByRole[2] as $user) <!-- Loop door gebruikers met roles_id 2 -->
...
@if(array_key_exists(3, $usersByRoleArray))
    <h2>Goedkeuren/Afkeuren</h2>
    @foreach($usersByRole[3] as $user) <!-- Loop door gebruikers met roles_id 3 -->
        <div class="card w-50 mb-3">
            <div class="card-body d-flex justify-content-between align-items-start">
                <div>
                    <h5 class="card-title">{{ $user->name }}</h5>
                ...
                <div>
                    <form action="{{ route('changeRole', ['id' => $user->id]) }}" method="POST">
                        @csrf
